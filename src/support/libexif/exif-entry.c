...
exif_entry_dump (ExifEntry * e, unsigned int indent)
{
    ...
    printf ("%sTag: 0x%x ('%s')\n", buf, e->tag, exif_tag_get_name (e->tag));
    printf ("%s  Format: %i ('%s')\n", buf, e->format,
...
/*	bindtextdomain (GETTEXT_PACKAGE, LIBEXIF_LOCALEDIR); */
    ...
    switch (e->tag)
    {
	  ...
	  if (strlen ((char *) e->data + strlen ((char *) e->data) + 1))
	      strncat (v, e->data + strlen (e->data) + 1, sizeof (v));
	  else
	  ...
	  d = 0.;
	  entry =
	      exif_content_get_entry (e->parent->parent->ifd[EXIF_IFD_0],
				      EXIF_TAG_MAKE);
	  if (entry && entry->data && !strncmp (entry->data, "Minolta", 7))
	  {
	      entry =
		  exif_content_get_entry (e->parent->parent->ifd[EXIF_IFD_0],
					  EXIF_TAG_MODEL);
	  ...
	  snprintf (v, sizeof (v), "%.1f mm%s",
		    (float) v_rat.numerator / (float) v_rat.denominator, b);
	  break;
	  ...
	  if (d < 1)
	      snprintf (v, sizeof (v), _("1/%d sec."), (int) (1. / d));
	  else
	  ...
	  switch (v_short)
	  {
		...
		break;
	    case 3:
		strncpy (v, _("Tungsten (incandescent light)"), sizeof (v));
		break;
	    case 4:
	  ...
	  snprintf (v, sizeof (v), "%s%.01f",
		    ...
		    (double) v_srat.numerator / (double) v_srat.denominator);
	  break;
	  ...
	  v_short = exif_get_short (e->data, o);
	  v_short2 =
	      exif_get_short (e->data + exif_format_get_size (e->format), o);
	  ...
	  switch (e->format)
	  {
		...
		snprintf (v, sizeof (v), "%i/%i",
			  (int) v_rat.numerator, (int) v_rat.denominator);
		for (i = 1; i < e->components; i++)
		{
		    v_rat = exif_get_rational (e->data + 8 * i, o);
		...
		v_srat = exif_get_srational (e->data, o);
		snprintf (v, sizeof (v), "%i/%i",
			  (int) v_srat.numerator, (int) v_srat.denominator);
		for (i = 1; i < e->components; i++)
		{
		    v_srat = exif_get_srational (e->data + 8 * i, o);
		    snprintf (b, sizeof (b), "%i/%i",
...
exif_entry_initialize (ExifEntry * e, ExifTag tag)
{
    ...
    switch (tag)
    {
	  ...
	  exif_set_short (e->data + exif_format_get_size (e->format), o, 8);
	  exif_set_short (e->data + 2 * exif_format_get_size (e->format),
			  o, 8);
	  break;
      case EXIF_TAG_YCBCR_SUB_SAMPLING:
	  e->components = 2;
	  ...
	  e->data = malloc (e->size);
	  exif_set_short (e->data, o, 2);
	  exif_set_short (e->data + exif_format_get_size (e->format), o, 1);
	  break;
	  ...
	  exif_set_rational (e->data, o, r);
	  r.numerator = 255;
	  exif_set_rational (e->data + exif_format_get_size (e->format), o,
			     r);
	  r.numerator = 0;
	  exif_set_rational (e->data + 2 * exif_format_get_size (e->format),
			     o, r);
	  r.numerator = 255;
	  exif_set_rational (e->data + 3 * exif_format_get_size (e->format),
			     o, r);
	  r.numerator = 0;
	  exif_set_rational (e->data + 4 * exif_format_get_size (e->format),
			     o, r);
	  r.numerator = 255;
	  exif_set_rational (e->data + 5 * exif_format_get_size (e->format),
			     o, r);
	  break;
